...
## UX-7 [minor] CheckButton-у-ActionRow -> AdwSwitchRow

10+ місць у dialogs/settings/{ui_tab,terminal_tab,secrets_tab,monitoring_tab,logging_tab}.rs:

    // було:
    let check = CheckButton::new();
    let row = adw::ActionRow::builder().title("Foo").build();
    row.add_suffix(&check);
    // стало:
    let row = adw::SwitchRow::builder().title("Foo").build();

Сигнал — connect_active_notify замість connect_toggled.
...
| Release | Items |
|---------|-------|
| 0.14.2 (hotfix) | див. CHANGELOG `[Unreleased]` |
| 0.15.0 | ARCH-1, ARCH-2, ARCH-4 (semver-break), CLI-1 wave 1, UX-4, UX-13, DOC-1 |
...
| 0.16.x | UX-3, UX-6..UX-12, CLI-3, CLI-4, TEST-1, CODE-1 |
