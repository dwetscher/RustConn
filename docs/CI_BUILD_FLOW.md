...
```mermaid
graph TD
    Tag[git push tag v0.16.9] --> BuildDeb[Build .deb]
    Tag --> BuildRPM[Build .rpm]
    Tag --> BuildAppImage[Build AppImage]
    Tag --> BuildFlatpak[Build .flatpak]

    BuildDeb --> |ubuntu-24.04| DebArtifact[rustconn_0.16.9_amd64.deb]
    BuildRPM --> |fedora:44 container| RPMArtifact[rustconn-0.16.9-1.fc44.x86_64.rpm]
    BuildAppImage --> |ubuntu-24.04| AppImageArtifact[RustConn-0.16.9-x86_64.AppImage]
    BuildFlatpak --> |GNOME 50 container| FlatpakArtifact[RustConn-0.16.9.flatpak]
